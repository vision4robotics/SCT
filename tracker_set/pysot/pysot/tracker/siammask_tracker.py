    ...
    def track(self, img):
        ...
        with torch.no_grad():
            if self.args.enhance:
                ...
                _,img,_ = self.DCE_net(img)
                img = img.squeeze(0)
                x_crop = img.permute(1,2,0)                    
                #img = img.cpu().detach().numpy() #现在只是粗略的全局增强，img会先从numpy到torch再到numpy，后面跟踪的时候还会到torch，这里是耗时的
