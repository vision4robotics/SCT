...
from pytracking.features.Enhancer.SCT_model import SCT as enhancer
class NetWithBackbone(NetWrapper):
    ...
    def __init__(self, net_path, use_gpu=True, initialize=False, image_format='rgb',
        ...
        self.enhancer = enhancer(img_size=128, embned_dim=32, win_size=4, token_embed='linear', token_mlp='res').cuda()
        self.enhancer.load_state_dict(torch.load('features/Enhancer/model_latest.pth'))
    def initialize(self, image_format='rgb', mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
        super().initialize()

    def preprocess_image(self, im: torch.Tensor,SCT_net):
