...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict, SCT_net) -> dict:
        ...
        init_backbone_feat = self.generate_init_samples(im, SCT_net)
    ...
    def track(self, image, SCT_net, info: dict = None) -> dict:
        self.debug_info = {}
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor, SCT_net):
        ...
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone(im_patches, SCT_net)
    ...
    def generate_init_samples(self, im: torch.Tensor, SCT_net) -> TensorList:
        ...
        im_patches = sample_patch_transformed(im, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms, SCT_net)

        # Extract initial backbone features
        with torch.no_grad():
            init_backbone_feat = self.net.extract_backbone(im_patches, SCT_net)
