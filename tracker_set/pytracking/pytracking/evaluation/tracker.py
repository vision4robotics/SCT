...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None, if_rt: int=0):
    ...
    def run_sequence(self, seq, SCT_net, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        if not self.if_rt:
            output = self._track_sequence(tracker, seq, init_info, SCT_net)
        else:
            output = self._track_sequence_rt(tracker, seq, init_info)
        return output

    def _track_sequence(self, tracker, seq, init_info, SCT_net):
        # Define outputs
        ...
        out = tracker.initialize(image, init_info,SCT_net)
        if out is None:
            out = {}
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            out = tracker.track(image, SCT_net, info)
            prev_output = OrderedDict(out)
    ...
    def _track_sequence_rt(self, tracker, seq, init_info, fps=30):
        # Define outputs
        # Each field in output is a list containing tracker prediction for each frame.
        ...
        while 1:
            ...
            if fidx == 0:
                ...
                image = self._read_image(seq.frames[fidx])
                info = seq.frame_info(fidx)
                info['previous_output'] = prev_output
